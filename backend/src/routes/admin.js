...
import { deleteFromGitee } from "../utils/gitee.js";
...
// 设置用户角色
router.patch("/users/:id/set-role", auth, admin, async (req, res) => {
  try {
    const { role } = req.body;
    ...
    res.json({ message: `已${role === "admin" ? "授予" : "撤销"}管理员权限` });
  } catch (err) {
    res.status(500).json({ message: "操作失败", error: err.message });
  }
});
...
router.post("/equipment", auth, admin, async (req, res) => {
  try {
    ...
    if (!name || !total)
      return res.status(400).json({ message: "名称和数量必填" });
  ...
  try {
    ...
    const eq = await prisma.equipment.update({
      ...
      data: {
        ...
        category,
        total: parseInt(total),
        available: Math.max(0, old.available + diff),
      },
    });
    res.json(eq);
  } catch (err) {
  ...
  try {
    ...
    const content = await prisma.siteContent.upsert({
      ...
      update: {
        ...
        introBox2,
        introRules,
        seatUserEditable: seatUserEditable !== false,
      },
      create: {
        id: 1,
        introTitle,
...
router.delete("/resources/:id", auth, admin, async (req, res) => {
  try {
    ...
    // 同步删除 Gitee 仓库中的文件（不阻塞，失败不影响删除）
    if (resource.fileUrl) {
      deleteFromGitee(resource.fileUrl).catch((err) =>
        console.warn("[Admin] Gitee 删除失败:", err.message),
      );
    }
